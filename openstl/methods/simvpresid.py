...
from openstl.models import SimVP_Model, SimVPGAN_Model, UNet_Model
...
class SimVPResid(Base_method):
    ...
    def __init__(self, args, device, steps_per_epoch):
        ...
        self.adapt_weights = torch.tensor([1,1,1,0,1])
        n_steps = 100
        y_50 = 0.01
    ...
    def train_one_epoch(self, runner, train_loader, epoch, num_updates, eta=None, **kwargs):
        ...
        for batch_x, batch_y, batch_static in train_pbar:
            ...
            with self.amp_autocast():
                ...
                #recon_loss = loss
                #encoded_norms = loss
                pred_y, resid = self._predict(batch_x, batch_y)
                # prepend batch_y[:,0,:,:,:] to pred_y along dimension 1
                _, total_loss, mse_loss,mse_div,std_div,reg_loss, sum_loss = self.criterion(pred_y[:,:,4:5,:,:], resid[:,:,4:5,:,:], batch_static)
            ...
            if not self.dist:
                ...
                losses_reg_m.update(mse_div.item(), batch_x.size(0))
                losses_div_m.update(std_div.item(), batch_x.size(0))
                losses_div_s.update(reg_loss.item(), batch_x.size(0))
